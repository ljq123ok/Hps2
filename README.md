...
> **当前测试版：v0.11**
>
> 开发测试设备：**Pure X View**。欢迎在 GitHub Issues 反馈测试结果。
...
- 已在开发测试手机 **Pure X View** 上完成过核心启动、BIOS/游戏运行、OpenGL 画面、横屏布局、虚拟按键、3 倍内部渲染倍率等阶段性验证。
- 本次发布包仅增加 native 构建路径脱敏，不改变模拟器功能逻辑；但发布前重新构建的这份 HAP 尚未由开发者重新自签后再次完成全流程真机回归。
- HAP 不包含 BIOS 或游戏镜像；用户必须使用自己的合法文件。
- HAP 未签名，用户必须用自己的开发者账号生成**调试签名**后安装；发布签名可能导致 JIT 不可用。
- 蓝牙手柄映射设置、虚拟按键布局自定义等功能仍在推进中。
...
- **蓝牙手柄未接入**（当前用屏幕虚拟手柄）
- 画面尺寸/缩放仍在调整（`docs/stage4-resolution-issue.md`）
- 多核优化未做（MTVU 等当前关闭）
